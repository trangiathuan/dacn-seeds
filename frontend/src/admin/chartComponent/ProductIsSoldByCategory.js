...
const ProductSalesByCategoryChart = () => {
        ...
        const fetchCategories = async () => {
            try {
                ...
                if (response.data.length > 0) {
                    ...
                    fetchProductSales(response.data[0]._id); // Tự động lấy thống kê của category đầu tiên
                }
            } catch (error) {
    ...
    const handleCategoryIdChange = (event) => {
        ...
        fetchProductSales(selectedCategoryId); // Gọi API để lấy thống kê ngay khi người dùng chọn
    };

    const fetchProductSales = async (selectedCategoryId) => {
        ...
        try {
            const response = await axios.get(`${API_URL}/sales-by-category/${selectedCategoryId}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
            ...
            if (!data.products || data.products.length === 0) {
                setError('Không có sản phẩm đã bán trong danh mục này.');
                return;
            }
                ...
                datasets: [
                    {
                        ...
                        data: data.products.map(product => product.value), // Sử dụng isSold làm số lượng sản phẩm đã bán
                        backgroundColor: 'rgba(75, 192, 192, 0.7)', // Màu xanh lá cây cho cột
                        borderColor: 'rgba(75, 192, 192, 1)', // Đổi màu viền cột thành màu đỏ
                        borderWidth: 1,
                    },
                ],
            });
    ...
    return (
        <div>
            <h2 className='text-center mt-3 '>THỐNG KÊ SỐ LƯỢNG SẢN PHẨM ĐÃ BÁN</h2>
...
export default ProductSalesByCategoryChart;
